(() => {
  ...
  function handleViewportChange() {
    if (isMobileFiltersMode() && isTextEntryFocused()) {
      return;
    }
  ...
  function initSearchableSelects() {
    ...
    document.querySelectorAll(".searchable-select").forEach((root) => {
      ...
      const displayPlaceholder = buildSearchableDisplayPlaceholder(select, input);
      const searchPlaceholder = buildSearchableSearchPlaceholder(select, input, root);
      ...
      input.addEventListener("input", () => {
        if (select.disabled) return;
        if (isSearchableMobileMode()) return;
        openSearchableSelect(instance);
  ...
  function syncSearchableControlState(instance) {
    ...
    instance.input.readOnly = !isDisabled && isSearchableMobileMode();
    instance.root.classList.toggle("is-disabled", isDisabled);
    instance.root.classList.toggle("is-mobile-picker", !isDisabled && isSearchableMobileMode());
  ...
  function openSearchableSelect(instance) {
    ...
    instance.input.readOnly = isSearchableMobileMode();
  ...
  function syncSearchableDropdownMode(instance) {
    const useMobileSearch = isSearchableMobileMode();
  ...
  function getSearchableSearchTerm(instance) {
    return isSearchableMobileMode() ? instance.mobileSearchTerm : instance.input.value;
  }

  function isSearchableMobileMode() {
  ...
  function enhanceNativeSearchableSelects() {
    ...
    document.querySelectorAll(AUTO_SEARCHABLE_SELECT_SELECTOR).forEach((select) => {
      ...
      const placeholder = buildSearchableDisplayPlaceholder(select);
      if (placeholder) {
  ...
  function buildSearchableDisplayPlaceholder(select, input = null) {
  ...
  function buildSearchableSearchPlaceholder(select, input = null, root = null) {
    const explicitPlaceholder = root?.dataset?.placeholder?.trim();
